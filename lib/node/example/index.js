...
const country = pivot.deriveDimension(fulham_1.squad, 'country');
const position = pivot.deriveDimension(fulham_1.squad, 'position');
// create the pivot cube from the squad data using position and country for x and y axes
const cube = pivot.cube(fulham_1.squad, { x: position, y: country });
...
console.log(`\t${position.map(c => print(c.data[0].value)).join('\t')}`);
result.forEach((row, i) => console.log(`${print(country[i].data[0].value)}\t${row.map(print).join('\t')}`));
