...
const y = pivot.dimension('country', fulham_1.squad.map(player => player.country).filter((value, index, source) => source.indexOf(value) === index).sort(), label('country'));
...
function label(key) {
    return (value) => Object.assign((row) => row[key] === value, {
        metadata: value
    });
}
