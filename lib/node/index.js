...
const pivotImplementation = (source, first, second, ...others) => first.map(predicate => second ? pivotImplementation(partition(source, predicate), second, ...others) : partition(source, predicate));
// partition an array in two, returning those that match the predicate.
const partition = (source, predicate) => {
    let remaining = 0;
