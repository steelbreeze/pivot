 ...
 */
const dimension = (key, values, callback = value => Object.assign((record) => record[key] === value, { metadata: [{ key, value }] })) => values.map(callback);
 ...
 */
const cube = (source, y, x) => y.map(slicer([...source])).map(slice => x.map(slicer(slice)));
exports.cube = cube;
/**
