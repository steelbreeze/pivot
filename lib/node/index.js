 ...
 */
const distinct = (table, key, getValue = row => row[key]) => [...table.reduce((result, row, index) => result.add(getValue(row, index, table)), new Set())];
 ...
 */
const dimension = (values, key, criteria = value => [{ key, value, predicate: row => row[key] === value }]) => values.map(criteria);
 ...
 */
const cube = (table, axes) => (0, exports.slice)(axes.y)(table.slice()).map((0, exports.slice)(axes.x));
exports.cube = cube;
/**
 ...
 */
const slice = (dimension) => table => dimension.map(criteria => split(table, row => criteria.every(criterion => criterion.predicate(row))));
exports.slice = slice;
/**
 ...
 * @private
 */
function split(source, predicate) {
    let result = [], i = 0, j = 0;
