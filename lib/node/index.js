 ...
 */
function slice(values, dimension) {
    return map(dimension, (predicate) => filter(values, predicate));
}
function pivot(values, ...[first, second, ...others]) {
    return second ? map(slice(values, first), (sliced) => pivot(sliced, second, ...others)) : slice(values, first);
}
/**
