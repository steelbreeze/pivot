 ...
 */
const distinct = (table, getValue) => [...new Set(table.map(getValue))];
 ...
 */
const dimension = (values, key, createCriteria = (value) => ({ predicate: (row) => row[key] === value, metadata: [{ key, value }] })) => values.map(createCriteria);
...
const slice = (dimension) => (source) => dimension.map((criteria) => {
    ...
    let length = 0, result = source.filter((row) => criteria.predicate(row) || !(source[length++] = row));
