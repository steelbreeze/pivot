 ...
 */
const distinct = (table, key, getValue = (row) => row[key]) => table.reduce((result, row) => {
 ...
 */
const dimension = (values, key, createCriteria = (value) => [Object.assign((row) => row[key] === value, { key, value })]) => values.map(createCriteria);
 ...
 */
const slice = (dimension) => (source) => dimension.map((criteria) => {
    // perform the filter; for items that don't pass the criteria, pack them at the start of the source
    let length = 0, result = source.filter((row) => criteria.every((criterion) => criterion(row)) || !(source[length++] = row));
