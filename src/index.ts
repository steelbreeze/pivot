 ...
 * @returns Returns an n-cube; minimally the source is returned if no dimensions passed, a Matrix if only one dimension passed, a Cube if two dimensions passed, and so one as more dimensions added.
 */
export const pivot = <TSource>(source: Array<TSource>, [first, ...remaining]: Array<Dimension<TSource>>): Array<any> =>
	first ? first.map(predicate => remaining.length ? pivot(source.filter(predicate), remaining) : source.filter(predicate)) : source;
